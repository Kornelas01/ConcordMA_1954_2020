...
#write_csv(one_to_one_TSN, "one_to_one_TSN.csv")
```

```{r}
one_to_one_TSN <- read_csv("one_to_one_TSN.csv")
...
```{r}
# Possibly works with the `which()` function once read in from .csv and not when straight from code?
...
### run this after top for loop is confirmed to work; need to deal with a "sp." name
for (i in 1:nrow(tsn_df)){
...
Concord_corrected_name <- read_csv("corrected_taxonomy.csv")
```
...
for (i in 1:nrow(Concord_long)) {
  if (str_trim(Concord_long$Scientific_name[i]) %in% misspelled){
    ...
    Concord_long$Scientific_name[i] <- Concord_corrected_name$Scientific_name[index]
  } else {
