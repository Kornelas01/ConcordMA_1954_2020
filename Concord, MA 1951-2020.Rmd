...
for (i in 1:nrow(tsn_df)) {
  if (is.na(tsn_df$TSN[i])) {
    ...
    if (length(index) > 0) {
      ...
      # Handle the case where there is no match if necessary
      # e.g., tsn_df$TSN[i] <- NA or tsn_df$TSN[i] <- some_default_value
    }
  }
  # The else block is unnecessary since it doesn't change anything.
}
...
for (i in 1:nrow(Concord_long)) {
  if (str_trim(Concord_long$Scientific_name[i]) %in% misspelled) {
    ...
    if (length(index) > 0) {
      ...
      # Handle the case where there is no match, if necessary
      # e.g., Concord_long$Scientific_name[i] <- Concord_long$Scientific_name[i]
    }
  } 
  # The else block is unnecessary since it does nothing.
}
...
Final step
```{r}
Clean_Concord_Data <- full_join(Concord_long, tsn_df, relationship = "many-to-many")
)

NA_TSN_Clean <- Clean_Concord_Data[is.na(Clean_Concord_Data$TSN), ] 
